...
function get_amount($amount = 0){
    $show_price = '';
    $option = config('getfund_option');

    $currency_position = $option->currency_position;
    $currency_sign = get_currency_symbol($option->currency_sign);
...
if ( ! function_exists('get_stripe_amount')) {
    function get_stripe_amount($amount = 0, $iso_3digit_currency = 'USD'){
        if ( ! $amount){
            return $amount;
        }
        ...
        $iso_3digit_currency = strtoupper($iso_3digit_currency);

        if (in_array($iso_3digit_currency, $non_decimal_currency)) {
            return $amount;
        }
