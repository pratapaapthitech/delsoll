                    ...
                    <div class="row">
                        <div class="col-md-10 col-xs-12">
                                ...
                                <div class="col-sm-8">
                                    <select class="form-control select2" name="fund_category_id" id="fund_category_id">
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" @if($fundraiser->fund_category_id == $category->id) selected="selected" @endif >{{ $category->category_name }}</option>
                                        @endforeach
                                    </select>
							...
							<div class="form-group {{ $errors->has('fund_title')? 'has-error':'' }}">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="fund_title" value="{{ $fundraiser->fund_title }}" name="fund_title" placeholder="@lang('app.title_name_fundraiser')">
							...
							<div class="form-group {{ $errors->has('fund_sub_title')? 'has-error':'' }}">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="fund_sub_title" value="{{ $fundraiser->fund_sub_title }}" name="fund_sub_title" placeholder="@lang('app.sub_title_fundraiser')">
							...
							<div class="form-group {{ $errors->has('fund_goal_ammount')? 'has-error':'' }}">
                                ...
                                <div class="col-sm-8">
                                    <input type="number" class="form-control" id="fund_goal_ammount" value="{{ $fundraiser->fund_goal_ammount }}" name="fund_goal_ammount" placeholder="@lang('app.goal_amount')">
                                </div>
                            </div>
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
                                    <input type="checkbox" name="fund_tax_exempt"  id="fund_tax_exempt" @if($fundraiser->fund_tax_exempt == 1) checked="checked" @endif>
                                </div>
                            </div>
							...
							<div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <label>
                                        <input type="radio" name="fund_begin_type"  value="1" onclick="showHideDiv(1);" @if($fundraiser->fund_begin_type == '1') checked="checked" @endif > @lang('app.start_immediately')
                                    </label> <br />
                                    <label>
                                        <input type="radio" name="fund_begin_type" value="2" onclick="showHideDiv(2);" @if($fundraiser->fund_begin_type == '2') checked="checked" @endif > @lang('app.start_date_specify')
                                    </label>
                                </div>
                            </div>
							
							<div class="form-group" id="begin_date_div" @if($fundraiser->fund_begin_type == '1') style="display:none" @endif>
                                <label for="fund_begin_date" class="col-sm-4 control-label">@lang('app.select_date_specify') </label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="fund_begin_date" value="{{ $fundraiser->fund_begin_date }}" name="fund_begin_date" placeholder="@lang('app.select_date_specify')">
                                </div>
                            </div>
							...
							<div class="form-group">
                                <div class="col-sm-12">
									...
									<div>@if($fundraiser->fund_logo_image) <img width="200" src="{{ $fundraiser->fund_logo_image_url()}}" /> @endif</div>
									<input type="hidden" name="hid_fund_logo_image" value="{{ $fundraiser->fund_logo_image }}">
                                </div>
                                <div class="col-sm-12">
							...
							<div class="form-group">
                                <div class="col-sm-12">
									...
									<div>@if($fundraiser->fund_banner_image) <img width="200" src="{{ $fundraiser->fund_banner_image_url()}}" /> @endif</div>
									<input type="hidden" name="hid_fund_banner_image" value="{{ $fundraiser->fund_banner_image }}">
                                </div>
                                <div class="col-sm-12">
							...
							<div class="form-group">
                                <div class="col-sm-12">
									...
									<div>@if($fundraiser->fund_own_image) <img width="200" src="{{ $fundraiser->fund_own_image_url()}}" /> @endif</div>
									<input type="hidden" name="hid_fund_own_image" value="{{ $fundraiser->fund_own_image }}">
                                </div>
                                <div class="col-sm-12">
							...
							@if($slider->fi_image) 
								<p>
									<img width="200" src="{{ $slider->fi_image_url()}}" />
									<a href="{{route('slider_image_delete', $slider->id)}}" class="btn btn-delete btn-danger btn-sm" data-toggle="tooltip" title="@lang('app.delete')"><i class="fa fa-trash-o"></i> </a>
								</p>
							@endif
							@endforeach
							</div>
							...
							<div class="alert alert-info">
								...
								<input type="hidden" id="hid_td_count" value="{{ $descriptions->count() }}">
                            </div>
							
							<div id="title_desc_divs">
								@if($descriptions->count())
								@foreach($descriptions as $key=>$description)
								<div class="form-group" id="td_{{ $key }}">
									<div class="col-sm-12">
										<label for="description" class="control-label"><b>Description</b>@if($key > 0)<a href="javascript:void(0);" onclick="removeDescriptionField('{{ $key }}')">Remove</a>@endif</label>
										<input type="text" class="form-control" name="fund_description_title[]" value="{{ $description->fd_title }}" placeholder="@lang('app.fund_description_title')"><br/>
										<textarea name="fund_description_description[]" class="form-control description" rows="5">{{ $description->fd_description }}</textarea>
									</div>
								</div>
								@endforeach
								@else
