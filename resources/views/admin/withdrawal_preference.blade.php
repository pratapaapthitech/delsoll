...
@section('content')
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            ...
                            <div class="form-group {{ $errors->has('default_withdrawal_account')? 'has-error':'' }}">
                                ...
                                <div class="col-sm-8">
                                    <fieldset>
                                        <label> <button type="button" class="btn btn-primary" onclick="goToStripe();">@lang('app.connect_with_stripe')</button> </label> <br />
							...
							<div class="config_wrap paypal-wrap">
                                ...
                                <div class="form-group">
                                    <label for="stripe_account_id" class="col-sm-7 control-label">@lang('app.stripe_account_id') : <b>{{ $userStripeAccountId }}</b></label>
                                </div>
                            ...
                            <!--<div class="form-group">
                                <div class="col-sm-8 col-sm-offset-4">
                                    <button type="button" class="btn btn-primary" onclick="goToStripe();">@lang('app.connect_with_stripe')</button>
                                </div>
                            </div>-->
	...
	<input type="hidden" id="stripeClientIdT" value="{{ $stripeClientIdT }}">
	<input type="hidden" id="stripeClientIdL" value="{{ $stripeClientIdL }}">
	<input type="hidden" id="stripeRedirectUrlT" value="{{ $stripeRedirectUrlT }}">
	<input type="hidden" id="stripeRedirectUrlL" value="{{ $stripeRedirectUrlL }}">
	<input type="hidden" id="stripeTestMode" value="{{ $stripeTestMode }}">
@endsection
...
@section('page-js')
    <script>
		...
		function goToStripe(){
			$stripeClientIdT = $('#stripeClientIdT').val();
			$stripeClientIdL = $('#stripeClientIdL').val();
			$stripeRedirectUrlT = $('#stripeRedirectUrlT').val();
			$stripeRedirectUrlL = $('#stripeRedirectUrlL').val();
			$stripeTestMode = $('#stripeTestMode').val();
			if($stripeTestMode == '1'){
				window.location = "https://connect.stripe.com/oauth/authorize?response_type=code&amp;client_id="+$stripeClientIdT+"&amp;scope=read_write&amp;redirect_uri="+$stripeRedirectUrlT;
			}else{
				window.location = "https://connect.stripe.com/oauth/authorize?response_type=code&amp;client_id="+$stripeClientIdL+"&amp;scope=read_write&amp;redirect_uri="+$stripeRedirectUrlL;
			}
		}
    </script>
